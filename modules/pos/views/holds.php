...
<head>
    ...
    <title>Held Orders - <?php echo htmlspecialchars($tenantName ?? 'POS'); ?></title>
    ...
    <div class="fade-in">
        <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 40px;">
            <div class="pos-title">
                <div style="display: inline-flex; align-items: center; gap: 8px; margin-bottom: 12px; background: #fffbeb; padding: 8px 16px; border-radius: 12px; color: #d97706; font-weight: 800; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">
                    <i class="fas fa-pause-circle"></i> Transaction Queue
                </div>
                <h1>Held Orders History</h1>
                <p>Review and resume sales that were put on standby for various reasons.</p>
            </div>
            <a href="<?php echo htmlspecialchars($posUrl('pos')); ?>" class="btn btn-primary" style="padding: 14px 28px;">
                <i class="fas fa-plus-circle"></i> Start New Checkout
            </a>
        </div>
        ...
        <?php if (!count($heldOrders ?? [])): ?>
            <div class="pos-card" style="padding: 100px 40px; text-align: center; border: 2px dashed var(--pos-border); background: transparent;">
                ...
                <h3 style="font-weight: 900; color: var(--pos-text);">No orders on standby</h3>
                <p style="color: var(--pos-text-muted); font-size: 16px; margin-top: 8px;">All transactions are currently finalized or active.</p>
            </div>
        <?php else: ?>
            <div style="max-width: 1000px; margin: 0 auto;">
                <?php foreach ($heldOrders as $o): ?>
                    <?php
                        ...
                        $cust = $o['customer_name'] ?? 'Walk-in Customer';
                    ...
                    <div class="hold-card">
                        ...
                        <div style="display: flex; align-items: center; gap: 40px;">
                            <div style="text-align: right;">
                                <div style="font-size: 11px; font-weight: 800; color: var(--pos-text-muted); text-transform: uppercase; margin-bottom: 4px;">Value</div>
                            ...
                            <div style="display: flex; gap: 10px;">
                                <a href="<?php echo htmlspecialchars($posUrl('pos?resume=' . $id)); ?>" class="btn btn-primary" style="padding: 12px 24px; font-weight: 900;">
                                    <i class="fas fa-play" style="font-size: 12px; margin-right: 8px;"></i> Resume
                                </a>
