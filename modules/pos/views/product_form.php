...
<head>
    ...
    <title>Product Management - <?php echo htmlspecialchars($tenantName ?? 'POS'); ?></title>
...
<body class="pos-app">
    ...
    <div class="fade-in">
        <div style="text-align: center; margin-bottom: 40px;">
            ...
            <h1 style="font-size: 36px; font-weight: 900; color: var(--pos-text); margin: 0;"><?php echo isset($product) ? 'Record Refinement' : 'New Product Entry'; ?></h1>
            <p style="color: var(--pos-text-muted); margin-top: 8px; font-size: 16px;">Configure your item specifications and stock levels below.</p>
        </div>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <section style="margin-bottom: 40px;">
                    ...
                    <div class="pos-form-group">
                        <label class="pos-form-label">Full Product Name <span style="color:red;">*</span></label>
                        <input type="text" name="name" class="pos-form-control" value="<?php echo htmlspecialchars($product['name'] ?? ''); ?>" required placeholder="Enter descriptive name...">
                    </div>
                    <div class="pos-grid cols-2" style="margin-top: 24px;">
                        <div class="pos-form-group">
                            <label class="pos-form-label">Classification / Category</label>
                        ...
                        <div class="pos-form-group">
                            <label class="pos-form-label">Status</label>
                            <select name="status" class="pos-form-control pos-form-select">
                                <option value="active" <?php echo (!isset($product) || $product['status'] == 'active') ? 'selected' : ''; ?>>Active (Visible)</option>
                                <option value="inactive" <?php echo (isset($product) && $product['status'] == 'inactive') ? 'selected' : ''; ?>>Hidden (Archived)</option>
                            </select>
                        </div>
                    </div>
                </section>
                ...
                <section style="margin-bottom: 40px;">
                    ...
                    <div class="pos-grid cols-2">
                        <div class="pos-form-group">
                            <label class="pos-form-label">Retail Price ($) <span style="color:red;">*</span></label>
                            <input type="number" name="price" step="0.01" class="pos-form-control" value="<?php echo $product['price'] ?? ''; ?>" required placeholder="0.00">
                        </div>
                        <div class="pos-form-group">
                            <label class="pos-form-label">Opening Stock Quantity <span style="color:red;">*</span></label>
                    ...
                    <div class="pos-grid cols-2" style="margin-top: 24px;">
                        <div class="pos-form-group">
                            <label class="pos-form-label">SKU / Reference ID</label>
                        ...
                        <div class="pos-form-group">
                            <label class="pos-form-label">Barcode Number (EAN/UPC)</label>
                            <input type="text" name="barcode" class="pos-form-control" value="<?php echo htmlspecialchars($product['barcode'] ?? ''); ?>" placeholder="Scan product barcode">
                        </div>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div class="pos-form-group">
                        <label class="pos-form-label">Featured Image</label>
                        <div class="upload-zone" onclick="document.getElementById('image-input').click()">
                            ...
                            <div id="upload-placeholder" style="<?php echo (isset($product) && $product['image']) ? 'display:none;' : ''; ?>">
                                ...
                                <div style="font-weight: 800; color: var(--pos-text); font-size: 15px;">Click to select or drag item image</div>
                                <div style="font-size: 13px; color: var(--pos-text-muted); margin-top: 6px; font-weight: 500;">Optimal size: 800x800px (JPG/PNG)</div>
                            </div>
                            <div id="image-preview-container" style="<?php echo (isset($product) && $product['image']) ? '' : 'display:none;'; ?>">
                                <?php if (isset($product) && $product['image']): ?>
                                    ...
                                    <p style="margin-top: 12px; font-size: 12px; color: var(--pos-text-muted); font-weight: 700;">Click image to upload a different one</p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    <div class="pos-form-group" style="margin-top: 32px;">
                        <label class="pos-form-label">Technical Description</label>
                        <textarea name="description" class="pos-form-control" rows="5" style="resize: none;" placeholder="Provide additional details about dimensions, materials or notes..."><?php echo htmlspecialchars($product['description'] ?? ''); ?></textarea>
                    </div>
                </section>
                ...
                <div style="display: flex; justify-content: flex-end; gap: 16px; margin-top: 48px; border-top: 1.5px solid var(--pos-border); padding-top: 32px;">
                    ...
                    <button type="submit" class="btn btn-primary" style="min-width: 200px; box-shadow: 0 10px 25px rgba(99, 102, 241, 0.3);">
                        <i class="fas fa-save" style="margin-right: 8px;"></i> <?php echo isset($product) ? 'Update Records' : 'Save Product'; ?>
                    </button>
                </div>
            </form>
        </div>
        ...
        function previewImage(input) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    previewCont.innerHTML = `<img src="${e.target.result}" class="preview-img"><p style="margin-top: 12px; font-size: 12px; color: var(--pos-text-muted); font-weight: 700;">Click image to upload a different one</p>`;
                };
